...
namespace Jeddsaliba\FilamentMessages\Models;
    ...
    protected function inboxTitle(): Attribute
    {
        return Attribute::make(
            get: function () {
                ...
                // Ambil daftar orang selain diri kita sendiri
                $otherParticipants = $userIds->filter(fn ($id) => $id != $authId);

                // JIKA CHAT DENGAN DIRI SENDIRI (Self-chat)
                if ($otherParticipants->isEmpty()) {
                    return Auth::user()?->name ?? 'Diri Sendiri';
                }

                // JIKA CHAT DENGAN ORANG LAIN
